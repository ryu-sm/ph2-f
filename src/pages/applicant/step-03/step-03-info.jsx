...
export const ApStep03Info = ({ stepIndex }) => {
  ...
  return (
    <ApConfirmGroup label={`STEP${stepIndex}：あなたのご職業`}>
      <ApConfirmItemGroup label={'ご職業'}>
        <Stack spacing={1} alignItems={'start'}>
          <Typography variant="modal_label" color={'text.main'}>
            {p_applicant_persons__0__office_occupation
              ? occupationOptions.find((item) => item.value === p_applicant_persons__0__office_occupation)?.label
              : 'ー'}
          </Typography>
          {p_applicant_persons__0__office_occupation === '99' && (
            <Typography variant="modal_label" color={'text.main'}>
              〈その他の詳細〉
              {p_applicant_persons__0__office_occupation_other ? p_applicant_persons__0__office_occupation_other : 'ー'}
            </Typography>
          )}
        </Stack>
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'業種'}>
        <Stack spacing={1} alignItems={'start'}>
          <Typography variant="modal_label" color={'text.main'}>
            {p_applicant_persons__0__office_industry
              ? industryOptions.find((item) => item.value === p_applicant_persons__0__office_industry)?.label
              : 'ー'}
          </Typography>
          {p_applicant_persons__0__office_industry === '99' && (
            <Typography variant="modal_label" color={'text.main'}>
              〈その他の詳細〉
              {p_applicant_persons__0__office_industry_other ? p_applicant_persons__0__office_industry_other : 'ー'}
            </Typography>
          )}
        </Stack>
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'職種'}>
        <Stack spacing={1} alignItems={'start'}>
          <Typography variant="modal_label" color={'text.main'}>
            {p_applicant_persons__0__office_occupation_detail
              ? occupationDetailOptions.find((item) => item.value === p_applicant_persons__0__office_occupation_detail)
                  ?.label
              : 'ー'}
          </Typography>
          {p_applicant_persons__0__office_occupation_detail === '99' && (
            <Typography variant="modal_label" color={'text.main'}>
              〈その他の詳細〉
              {p_applicant_persons__0__office_occupation_detail_other
                ? p_applicant_persons__0__office_occupation_detail_other
      ...
      <ApConfirmItemGroup label={'勤務先名'}>
        {p_applicant_persons__0__office_name_kanji ? p_applicant_persons__0__office_name_kanji : 'ー'}
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'所属部課'}>
        {p_applicant_persons__0__office_department ? p_applicant_persons__0__office_department : 'ー'}
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'勤務先の電話番号'}>
        {p_applicant_persons__0__office_phone ? p_applicant_persons__0__office_phone : 'ー'}
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'勤務先の住所'}>
        {p_applicant_persons__0__office_postal_code ? (
          <Stack spacing={1} alignItems={'start'}>
            ...
            <Typography variant="modal_label" color={'text.main'}>
              {`${p_applicant_persons__0__office_prefecture_kanji}${p_applicant_persons__0__office_city_kanji}${p_applicant_persons__0__office_district_kanji}`}
            </Typography>
            <Typography variant="modal_label" color={'text.main'}>
              {p_applicant_persons__0__office_other_address_kanji}
            </Typography>
      ...
      <ApConfirmItemGroup label={'従業員数'}>
        {p_applicant_persons__0__office_employee_num
          ? Number(p_applicant_persons__0__office_employee_num).toLocaleString()
          : 'ー'}
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'入社年月'}>
        {p_applicant_persons__0__office_joining_date
          ? formatJapanDate(p_applicant_persons__0__office_joining_date)
          : 'ー'}
      </ApConfirmItemGroup>
      <ApConfirmItemGroup label={'ご年収'}>
        {p_applicant_persons__0__last_year_income ||
        p_applicant_persons__0__last_year_bonus_income ||
        p_applicant_persons__0__income_sources.length ||
        p_applicant_persons__0__tax_return ||
        p_applicant_persons__0__tax_return_reasons.length ? (
          <Stack spacing={1} alignItems={'start'}>
            <Typography variant="modal_label" color={'text.main'}>
              〈前年度年収〉
              {p_applicant_persons__0__last_year_income
                ? `${Number(p_applicant_persons__0__last_year_income).toLocaleString()}万円`
                : 'ー'}
            </Typography>
            {isMCJ && (
              <Typography variant="modal_label" color={'text.main'}>
                〈うち、ボーナス（MCJ固有項目）〉
                {p_applicant_persons__0__last_year_bonus_income
                  ? `${Number(p_applicant_persons__0__last_year_bonus_income).toLocaleString()}万円`
                  : 'ー'}
              </Typography>
            )}
            <Typography variant="modal_label" color={'text.main'}>
            ...
            <Typography variant="modal_label" color={'text.main'}>
              〈確定申告をしていますか？〉
              {p_applicant_persons__0__tax_return
                ? taxReturnOptions.find((item) => item.value === p_applicant_persons__0__tax_return)?.label
                : 'ー'}
            </Typography>
            <Typography variant="modal_label" color={'text.main'}>
              〈確定申告の理由〉
      ...
      <ApConfirmItemGroup label={'現在、出向（派遣）していますか？'}>
        {p_applicant_persons__0__transfer_office ? (
          p_applicant_persons__0__transfer_office === '1' ? (
            <Stack spacing={1} alignItems={'start'}>
              {p_applicant_persons__0__transfer_office_name_kanji && (
                <Typography variant="modal_label" color={'text.main'}>
                  〈出向（派遣）勤務先名〉
              ...
              {p_applicant_persons__0__transfer_office_phone && (
                <Typography variant="modal_label" color={'text.main'}>
                  〈出向（派遣）先 電話番号〉
                  {p_applicant_persons__0__transfer_office_phone ? p_applicant_persons__0__transfer_office_phone : 'ー'}
                </Typography>
              )}
              <Typography variant="sp_value_text" color="b_333">
                ...
                {p_applicant_persons__0__transfer_office_postal_code ? (
                  <Stack spacing={1} alignItems={'start'} sx={{ px: 4, pt: 1 }}>
                    ...
                    <Typography variant="modal_label" color={'text.main'}>
                      {`${p_applicant_persons__0__transfer_office_prefecture_kanji}${p_applicant_persons__0__transfer_office_city_kanji}${p_applicant_persons__0__transfer_office_district_kanji}`}
                    </Typography>
                    <Typography variant="modal_label" color={'text.main'}>
                      {p_applicant_persons__0__transfer_office_other_address_kanji}
                    </Typography>
            ...
            <Typography variant="modal_label" color={'text.main'}>
              {p_applicant_persons__0__maternity_paternity_leave
                ? maternityPaternityLeaveOptions.find(
                    (item) => item.value === p_applicant_persons__0__maternity_paternity_leave
                  )?.label
                : 'ー'}
            </Typography>
            <Typography variant="modal_label" color={'text.main'}>
              〈取得開始時期〉
              {p_applicant_persons__0__maternity_paternity_leave_start_date
                ? formatJapanDate(p_applicant_persons__0__maternity_paternity_leave_start_date)
                : 'ー'}
            </Typography>
            <Typography variant="modal_label" color={'text.main'}>
              〈取得終了時期〉
              {p_applicant_persons__0__maternity_paternity_leave_end_date
                ? formatJapanDate(p_applicant_persons__0__maternity_paternity_leave_end_date)
                : 'ー'}
            </Typography>
          </Stack>
        ) : (
        ...
        <ApConfirmItemGroup label={'介護休暇の取得状況（MCJ固有項目）'}>
          {p_applicant_persons__0__nursing_leave ? (
            <Stack spacing={1} alignItems={'start'}>
              <Typography variant="modal_label" color={'text.main'}>
                {p_applicant_persons__0__nursing_leave
                  ? nursingLeaveOptions.find((item) => item.value === p_applicant_persons__0__nursing_leave)?.label
                  : 'ー'}
              </Typography>
            </Stack>
          ) : (
      ...
      {!agentSended && (
        <Stack alignItems={'center'} sx={{ py: 3 }}>
          <ApLighterButton
            ...
            onClick={() => navigate(routeNames.apStep03Page.path)}
          >
