...
export const ApStep03Info = ({ stepIndex }) => {
  ...
  const { isMCJ, p_applicant_persons__0 } = useRecoilValue(localApplication);
    ...
    <ApConfirmGroup stepIndex={stepIndex} label={`：あなたのご職業`}>
      ...
      <ApConfirmItemGroup label={'従業員数'}>
        {p_applicant_persons__0.office_employee_num
          ? formatNumber(p_applicant_persons__0.office_employee_num, '名')
          : 'ー'}
      </ApConfirmItemGroup>
          ...
          <Stack spacing={1} alignItems={'start'}>
            ...
            <Typography variant="modal_label" textAlign={'start'} color={'text.main'}>
              ...
              {p_applicant_persons__0.maternity_paternity_leave_start_date
                ? formatJapanDate(p_applicant_persons__0.maternity_paternity_leave_start_date, true)
                : 'ー'}
            </Typography>
            <Typography variant="modal_label" textAlign={'start'} color={'text.main'}>
              〈取得終了時期〉
              {p_applicant_persons__0.maternity_paternity_leave_end_date
                ? formatJapanDate(p_applicant_persons__0.maternity_paternity_leave_end_date, true)
                : 'ー'}
            </Typography>
          </Stack>
        ) : (
